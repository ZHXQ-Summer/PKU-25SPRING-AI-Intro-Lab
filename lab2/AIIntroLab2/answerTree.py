...
def entropy(Y: np.ndarray):
    ...
    for key in a.keys():
        if(a[key]!=0):
            en-=np.log(a[key]/n)*(a[key]/n)/np.log(2)
    return en
...
def gain(X: np.ndarray, Y: np.ndarray, idx: int):
    ...
    ret = 0
    # TODO: YOUR CODE HERE
    for i in range(ufeat.size):
        za=Y[feat==ufeat[i]]
        ret+=featp[i]*entropy(za)
    ret=entropy(Y)-ret
    return ret
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    # TODO: YOUR CODE HERE
    for i in range(ufeat.size):
        y_sub=Y[feat==ufeat[i]]
        x_sub=X[feat==ufeat[i]]
        child=buildTree(x_sub, y_sub, unused, depth-1, purity_bound, gainfunc, prefixstr="")
        root.children[ufeat[i]]=child
