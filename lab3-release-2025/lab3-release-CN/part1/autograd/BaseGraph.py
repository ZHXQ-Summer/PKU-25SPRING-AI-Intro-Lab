    ...
    def optimstep(self, lr, wd1, wd2):
        ...
        for n in self:
            for i, param in enumerate(n.params):
                if(i==0 and (isinstance(n,Linear))):
                    learning=lr*(n.grad[i]+2*n.params[i]*wd2+wd1*np.sign(n.params[i]))
                elif(i==1 and (isinstance(n,Linear))):
                    learning=lr*n.grad[i]
                else:
                    learning = lr*n.grad[i].sum(axis=0)
                param -= learning
    def parameters(self):
        """
        返回当前计算图中的所有节点的参数
