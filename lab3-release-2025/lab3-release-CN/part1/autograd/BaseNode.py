...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        scores/=math.sqrt(self.dim)
        attn_weights = softmax(scores, axis=-1)
        output = np.matmul(attn_weights, v) 
        output += X
        self.cache.append((X,attn_weights,q,k,v))
        return output
...
class Linear(Node):
    ...
    def cal(self, X):
        # TODO: YOUR CODE HERE
        self.cache.append(X.copy())
        A=np.matmul(X,self.params[0])
        ans=np.add(A,self.params[1])
        return ans
...
class ResLinear(Node):
    ...
    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        X=self.cache[-1]
        self.grad.append(((X.transpose(0, 2, 1))@grad))
        self.grad.append(np.sum(grad,axis=(0)))
        return (grad@self.params[0].T+grad)
...
class LogSoftmax(Node):
    ...
    def backcal(self, grad):
        ...
        return grad - np.multiply(grad.sum(axis=self.dim, keepdims=True), softmax)
