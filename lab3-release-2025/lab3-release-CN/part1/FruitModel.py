...
class NaiveBayesModel:
    ...
    def count(self):
        for i in range(self.dataset.len):
            tokens,label=self.dataset.__getitem__(i)
            for token in tokens:
                self.token_num[label][token]=self.token_num[label].get(token,0)+1
        vocabulary = set(self.token_num[0].keys()).union(set(self.token_num[1].keys()))
    ...
    def __call__(self, text):
        ...
        # 返回1或0代表当前句子分类为正/负样本
        tokenize_text=text
        score_0=math.log(self.pos_neg_num[0])
        score_1=math.log(self.pos_neg_num[1])
        for token in tokenize_text:
            p_0=math.log((self.token_num[0].get(token,0)+1)/(self.pos_neg_num[0]+len(tokenize_text)))
            score_0+=p_0
            p_1=math.log((self.token_num[1].get(token,0)+1)/(self.pos_neg_num[1]+len(tokenize_text)))
            score_1+=p_1
...
class Embedding():
    ...
    def __call__(self, text, max_len=50):
        ...
        vectors = []
        for token in tokens:
            vec = self.emb.get(token, np.zeros(100))
            vectors.append(vec)
...
class QAModel():
    ...
    def tf(self, word, document):
        ...
        cleaned_tokens = document['document']
        return cleaned_tokens.count(word)/len(cleaned_tokens)
    ...
    def __call__(self, query):
        ...
        for document in self.document_list:
            ti=0
            for word in query:
                ti+=self.tfidf(word,document)
            if(ti>max_tfidf):
                max_tfidf=ti
                best_document=document
        best_sentence=None
        max_idf=float('-inf')
        for s in best_document['sentences']:
            i=0
            for word in query:
                if word in s[0]:
                    i+=self.idf(word)
